...
from pipeline import on_pair as pipeline_on_pair
...
# Converte DexConfig → dict no config["DEXES"]
dexes = []
for d in raw_dexes:
...
async def handle_api_pair(pair_addr, token0, token1, dex_info):
    """
    Envolve pipeline_on_pair para enviar pelo loop do Telegram.
    """
    try:
        await pipeline_on_pair(pair_addr, token0, token1, dex_info)
    except Exception as e:
        logger.error("🚨 Erro no pipeline ao processar par: %s", e, exc_info=True)
...
async def start_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    texto = (
        "🎯 Sniper Bot\n\n"
        "/snipe — iniciar descoberta e estratégia\n"
        "/stop — parar sniper\n"
        "/sniperstatus — status do discovery\n"
...
cmds = [BotCommand(n, h.__doc__ or "") for n, h in handlers]
...
def iniciar_sniper():
    ...
    # Usa callback que envia dados ao pipeline dentro do loop do Telegram
    def _cb(pair_addr, token0, token1, dex_info):
        asyncio.run_coroutine_threadsafe(
            handle_api_pair(pair_addr, token0, token1, dex_info),
            telegram_loop
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--worker", action="store_true", help="Executar modo worker (discovery + trading)")
    ...
    if args.worker:
        # Modo worker: discovery → pipeline; e timer de saída
        logger.info("▶️ Iniciando Worker Mode")
        subscribe_new_pairs(callback=pipeline_on_pair)
        while True:
            try:
                coro = check_exits()
