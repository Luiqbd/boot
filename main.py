...
TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "0")  # usado no /test_notify
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    mensagem = (
        ...
        "🏓 /ping — Teste de vida.\n"
        "🛰️ /test_notify — Mensagem de teste.\n"        # <<< alterado
...
if __name__ == "__main__":
    ...
    application.add_handler(CommandHandler("sniper_status", sniper_status_cmd))   # <<< alterado
    application.add_handler(CommandHandler("ping", ping_cmd))
    application.add_handler(CommandHandler("test_notify", test_notify_cmd))      # <<< alterado
    ...
    async def start_bot():
        ...
        await application.bot.set_my_commands([
            ...
            BotCommand("sniper_status", "Status do sniper"),    # <<< alterado
            BotCommand("ping", "Teste de vida (pong)"),
            BotCommand("test_notify", "Envia uma notificação de teste")  # <<< alterado
        ])
