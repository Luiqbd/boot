...
def has_min_volume(dex_client: DexClient, token_in: str, token_out: str, min_volume_eth: float) -> bool:
    try:
        volume = dex_client.get_recent_volume(token_in, token_out)
        return float(volume) >= float(min_volume_eth)
    except Exception as e:
...
def is_honeypot(token_address: str, router_address: str, weth_address: str, test_amount_eth: float, strict: bool = False) -> bool:
    try:
        ...
        amt = int(Decimal(str(test_amount_eth)) * 10**18)
        out = router.functions.getAmountsOut(amt, [weth_address, token_address]).call()
        return len(out) < 2 or int(out[-1]) == 0
    except Exception as e:
...
async def on_new_pair(dex_info, pair_addr, token0, token1, bot=None, loop=None):
    ...
    try:
        ...
        if not is_contract_verified(target_token, API_KEY):
            safe_notify(bot, f"⚠️ Token {target_token} não verificado", loop)
            if BLOCK_UNVERIFIED:
                return

        if is_token_concentrated(target_token, API_KEY, TOP_HOLDER_LIMIT):
            safe_notify(bot, f"🚫 Token {target_token} concentração alta", loop)
            return
        ...
        log.info(
            f"[Pré-Risk] {token0}/{token1} preço={preco_atual} ETH | "
            f"size={amt_eth} ETH | slippage={slip_limit*100:.2f}%"
        )
        ...
        try:
            ...
            log.error(f"Falha ao criar executor de trade: {e}", exc_info=True)
            return
